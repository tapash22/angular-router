import {
  DisplayField,
  DisplayFieldWithIcon,
  User,
  UserMenu,
} from '../interfaces/user';
import {
  faUserGear,
  faMapLocation,
  faMailBulk,
  faMobilePhone,
} from '@fortawesome/free-solid-svg-icons';

export const MOCK_USERS: User[] = [
  {
    id: 1,
    name: 'Tapash Paul',
    email: 'tapash.paul@gmail.com',
    password: 'Tapas1@3',
    role: 'admin',
    department: 'IT',
    phone: '+1-555-0101',
    avatarUrl: 'https://i.pravatar.cc/150?img=1',
    status: 'active',
    joiningDate: '2020-01-15',
    location: 'New York, USA',
    performanceScore: 71,
    rating: 4.8,
    completedProjects: 25,
    ongoingProjects: 3,
    salary: 120000,
    lastEvaluationDate: '2024-12-20',
    totalTimeSpentHours: 2500,
    projects: [
      {
        id: 1,
        project_title: 'Website Redesign',
        project_subtitle: 'Modernize corporate website',
        project_project_length: 6,
        project_estimated_date: '2025-06-01',
        project_costing_needed: 50000,
        project_resource_needed: 5,
        project_requirement: [
          'UI/UX Design',
          'Frontend Development',
          'Backend Integration',
        ],
        working_resource: [
          {
            id: 1,
            name: 'Tapash Paul',
            email: 'tapash.paul@techcorp.com',
            time_spent_hours: 140,
            performance_score: 95,
          },
          {
            id: 2,
            name: 'Alice',
            email: 'alice@gmail.com',
            time_spent_hours: 120,
            performance_score: 88,
          },
        ],
        projectStatus: 'pause',
      },
      {
        id: 2,
        project_title: 'CRM Integration',
        project_subtitle: 'Connect internal tools with CRM',
        project_project_length: 4,
        project_estimated_date: '2025-04-15',
        project_costing_needed: 30000,
        project_resource_needed: 3,
        project_requirement: ['API Integration', 'Database Sync'],
        working_resource: [
          {
            id: 1,
            name: 'Tapash Paul',
            email: 'tapash.paul@techcorp.com',
            time_spent_hours: 100,
            performance_score: 90,
          },
        ],
        projectStatus: 'completed',
      },
      {
        id: 3,
        project_title: 'Website Redesign',
        project_subtitle: 'Modernize corporate website',
        project_project_length: 6,
        project_estimated_date: '2025-06-01',
        project_costing_needed: 50000,
        project_resource_needed: 5,
        project_requirement: [
          'UI/UX Design',
          'Frontend Development',
          'Backend Integration',
        ],
        working_resource: [
          {
            id: 1,
            name: 'Tapash Paul',
            email: 'tapash.paul@techcorp.com',
            time_spent_hours: 140,
            performance_score: 95,
          },
          {
            id: 2,
            name: 'Alice',
            email: 'alice@gmail.com',
            time_spent_hours: 120,
            performance_score: 88,
          },
        ],
        projectStatus: 'start',
      },
      {
        id: 4,
        project_title: 'CRM Integration',
        project_subtitle: 'Connect internal tools with CRM',
        project_project_length: 4,
        project_estimated_date: '2025-04-15',
        project_costing_needed: 30000,
        project_resource_needed: 3,
        project_requirement: ['API Integration', 'Database Sync'],
        working_resource: [
          {
            id: 1,
            name: 'Tapash Paul',
            email: 'tapash.paul@techcorp.com',
            time_spent_hours: 100,
            performance_score: 90,
          },
        ],
        projectStatus: 'completed',
      },
      {
        id: 5,
        project_title: 'Website Redesign',
        project_subtitle: 'Modernize corporate website',
        project_project_length: 6,
        project_estimated_date: '2025-06-01',
        project_costing_needed: 50000,
        project_resource_needed: 5,
        project_requirement: [
          'UI/UX Design',
          'Frontend Development',
          'Backend Integration',
        ],
        working_resource: [
          {
            id: 1,
            name: 'Tapash Paul',
            email: 'tapash.paul@techcorp.com',
            time_spent_hours: 140,
            performance_score: 95,
          },
          {
            id: 2,
            name: 'Alice',
            email: 'alice@gmail.com',
            time_spent_hours: 120,
            performance_score: 88,
          },
        ],
        projectStatus: 'start',
      },
      {
        id: 6,
        project_title: 'CRM Integration',
        project_subtitle: 'Connect internal tools with CRM',
        project_project_length: 4,
        project_estimated_date: '2025-04-15',
        project_costing_needed: 30000,
        project_resource_needed: 3,
        project_requirement: ['API Integration', 'Database Sync'],
        working_resource: [
          {
            id: 1,
            name: 'Tapash Paul',
            email: 'tapash.paul@techcorp.com',
            time_spent_hours: 100,
            performance_score: 90,
          },
        ],
        projectStatus: 'completed',
      },
    ],
  },
  {
    id: 2,
    name: 'Tapash Paul',
    email: 'paul@techcorp.com',
    password: 'Tapas1@3',
    role: 'manager',
    department: 'IT',
    phone: '+1-555-0101',
    avatarUrl: 'https://i.pravatar.cc/150?img=1',
    status: 'active',
    joiningDate: '2020-01-15',
    location: 'New York, USA',
    performanceScore: 91,
    rating: 3.8,
    completedProjects: 25,
    ongoingProjects: 3,
    salary: 120000,
    lastEvaluationDate: '2024-12-20',
    totalTimeSpentHours: 2500,
    projects: [
      {
        id: 1,
        project_title: 'Website Redesign',
        project_subtitle: 'Modernize corporate website',
        project_project_length: 6,
        project_estimated_date: '2025-06-01',
        project_costing_needed: 50000,
        project_resource_needed: 5,
        project_requirement: [
          'UI/UX Design',
          'Frontend Development',
          'Backend Integration',
        ],
        working_resource: [
          {
            id: 1,
            name: 'Tapash Paul',
            email: 'tapash.paul@techcorp.com',
            time_spent_hours: 140,
            performance_score: 95,
          },
          {
            id: 2,
            name: 'Alice',
            email: 'alice@gmail.com',
            time_spent_hours: 120,
            performance_score: 88,
          },
        ],
        projectStatus: 'start',
      },
      {
        id: 2,
        project_title: 'CRM Integration',
        project_subtitle: 'Connect internal tools with CRM',
        project_project_length: 4,
        project_estimated_date: '2025-04-15',
        project_costing_needed: 30000,
        project_resource_needed: 3,
        project_requirement: ['API Integration', 'Database Sync'],
        working_resource: [
          {
            id: 1,
            name: 'Tapash Paul',
            email: 'tapash.paul@techcorp.com',
            time_spent_hours: 100,
            performance_score: 90,
          },
        ],
        projectStatus: 'completed',
      },
    ],
  },
  {
    id: 3,
    name: 'newPaul',
    email: 'new@techcorp.com',
    password: 'Tapas1@3',
    role: 'officer',
    department: 'IT',
    phone: '+1-555-0101',
    avatarUrl: 'https://i.pravatar.cc/150?img=1',
    status: 'active',
    joiningDate: '2020-01-15',
    location: 'New York, USA',
    performanceScore: 91,
    rating: 4.8,
    completedProjects: 25,
    ongoingProjects: 3,
    salary: 120000,
    lastEvaluationDate: '2024-12-20',
    totalTimeSpentHours: 2500,
    projects: [
      {
        id: 1,
        project_title: 'Website Redesign',
        project_subtitle: 'Modernize corporate website',
        project_project_length: 6,
        project_estimated_date: '2025-06-01',
        project_costing_needed: 50000,
        project_resource_needed: 5,
        project_requirement: [
          'UI/UX Design',
          'Frontend Development',
          'Backend Integration',
        ],
        working_resource: [
          {
            id: 1,
            name: 'Tapash Paul',
            email: 'tapash.paul@techcorp.com',
            time_spent_hours: 140,
            performance_score: 95,
          },
          {
            id: 2,
            name: 'Alice',
            email: 'alice@gmail.com',
            time_spent_hours: 120,
            performance_score: 88,
          },
        ],
        projectStatus: 'start',
      },
      {
        id: 2,
        project_title: 'CRM Integration',
        project_subtitle: 'Connect internal tools with CRM',
        project_project_length: 4,
        project_estimated_date: '2025-04-15',
        project_costing_needed: 30000,
        project_resource_needed: 3,
        project_requirement: ['API Integration', 'Database Sync'],
        working_resource: [
          {
            id: 1,
            name: 'Tapash Paul',
            email: 'tapash.paul@techcorp.com',
            time_spent_hours: 100,
            performance_score: 90,
          },
        ],
        projectStatus: 'completed',
      },
    ],
  },
  {
    id: 4,
    name: 'demo',
    email: 'user@gmail.com',
    password: 'Tapas1@3',
    role: 'user',
    department: 'IT',
    phone: '+1-555-0101',
    avatarUrl: 'https://i.pravatar.cc/150?img=1',
    status: 'active',
    joiningDate: '2020-01-15',
    location: 'New York, USA',
    performanceScore: 91,
    rating: 4.8,
    completedProjects: 25,
    ongoingProjects: 3,
    salary: 120000,
    lastEvaluationDate: '2024-12-20',
    totalTimeSpentHours: 2500,
    projects: [
      {
        id: 1,
        project_title: 'Website Redesign',
        project_subtitle: 'Modernize corporate website',
        project_project_length: 6,
        project_estimated_date: '2025-06-01',
        project_costing_needed: 50000,
        project_resource_needed: 5,
        project_requirement: [
          'UI/UX Design',
          'Frontend Development',
          'Backend Integration',
        ],
        working_resource: [
          {
            id: 1,
            name: 'Tapash Paul',
            email: 'tapash.paul@techcorp.com',
            time_spent_hours: 140,
            performance_score: 95,
          },
          {
            id: 2,
            name: 'Alice',
            email: 'alice@gmail.com',
            time_spent_hours: 120,
            performance_score: 88,
          },
        ],
        projectStatus: 'start',
      },
      {
        id: 2,
        project_title: 'CRM Integration',
        project_subtitle: 'Connect internal tools with CRM',
        project_project_length: 4,
        project_estimated_date: '2025-04-15',
        project_costing_needed: 30000,
        project_resource_needed: 3,
        project_requirement: ['API Integration', 'Database Sync'],
        working_resource: [
          {
            id: 1,
            name: 'Tapash Paul',
            email: 'tapash.paul@techcorp.com',
            time_spent_hours: 100,
            performance_score: 90,
          },
        ],
        projectStatus: 'completed',
      },
    ],
  },
  {
    id: 5,
    name: 'demo 2',
    email: 'ultrawave@gmail.com',
    password: 'Tapas1@3',
    role: 'user',
    department: 'IT',
    phone: '+1-555-0101',
    avatarUrl: 'https://i.pravatar.cc/150?img=1',
    status: 'active',
    joiningDate: '2020-01-15',
    location: 'New York, USA',
    performanceScore: 91,
    rating: 4.8,
    completedProjects: 25,
    ongoingProjects: 3,
    salary: 120000,
    lastEvaluationDate: '2024-12-20',
    totalTimeSpentHours: 2500,
    projects: [
      {
        id: 1,
        project_title: 'Website Redesign',
        project_subtitle: 'Modernize corporate website',
        project_project_length: 6,
        project_estimated_date: '2025-06-01',
        project_costing_needed: 50000,
        project_resource_needed: 5,
        project_requirement: [
          'UI/UX Design',
          'Frontend Development',
          'Backend Integration',
        ],
        working_resource: [
          {
            id: 1,
            name: 'Tapash Paul',
            email: 'tapash.paul@techcorp.com',
            time_spent_hours: 140,
            performance_score: 95,
          },
          {
            id: 2,
            name: 'Alice',
            email: 'alice@gmail.com',
            time_spent_hours: 120,
            performance_score: 88,
          },
        ],
        projectStatus: 'start',
      },
      {
        id: 2,
        project_title: 'CRM Integration',
        project_subtitle: 'Connect internal tools with CRM',
        project_project_length: 4,
        project_estimated_date: '2025-04-15',
        project_costing_needed: 30000,
        project_resource_needed: 3,
        project_requirement: ['API Integration', 'Database Sync'],
        working_resource: [
          {
            id: 1,
            name: 'Tapash Paul',
            email: 'tapash.paul@techcorp.com',
            time_spent_hours: 100,
            performance_score: 90,
          },
        ],
        projectStatus: 'completed',
      },
    ],
  },
  {
    id: 6,
    name: 'officer',
    email: 'officer@gmail.com',
    password: 'Tapas1@3',
    role: 'officer',
    department: 'IT',
    phone: '+1-555-0101',
    avatarUrl: 'https://i.pravatar.cc/150?img=1',
    status: 'active',
    joiningDate: '2020-01-15',
    location: 'New York, USA',
    performanceScore: 91,
    rating: 4.8,
    completedProjects: 25,
    ongoingProjects: 3,
    salary: 120000,
    lastEvaluationDate: '2024-12-20',
    totalTimeSpentHours: 2500,
    projects: [
      {
        id: 1,
        project_title: 'Website Redesign',
        project_subtitle: 'Modernize corporate website',
        project_project_length: 6,
        project_estimated_date: '2025-06-01',
        project_costing_needed: 50000,
        project_resource_needed: 5,
        project_requirement: [
          'UI/UX Design',
          'Frontend Development',
          'Backend Integration',
        ],
        working_resource: [
          {
            id: 1,
            name: 'Tapash Paul',
            email: 'tapash.paul@techcorp.com',
            time_spent_hours: 140,
            performance_score: 95,
          },
          {
            id: 2,
            name: 'Alice',
            email: 'alice@gmail.com',
            time_spent_hours: 120,
            performance_score: 88,
          },
        ],
        projectStatus: 'start',
      },
      {
        id: 2,
        project_title: 'CRM Integration',
        project_subtitle: 'Connect internal tools with CRM',
        project_project_length: 4,
        project_estimated_date: '2025-04-15',
        project_costing_needed: 30000,
        project_resource_needed: 3,
        project_requirement: ['API Integration', 'Database Sync'],
        working_resource: [
          {
            id: 1,
            name: 'Tapash Paul',
            email: 'tapash.paul@techcorp.com',
            time_spent_hours: 100,
            performance_score: 90,
          },
        ],
        projectStatus: 'completed',
      },
    ],
  },
  {
    id: 6,
    name: 'user',
    email: 'user@gmail.com',
    password: 'Tapas1@3',
    role: 'user',
    department: 'IT',
    phone: '+1-555-0101',
    avatarUrl: 'https://i.pravatar.cc/150?img=1',
    status: 'active',
    joiningDate: '2020-01-15',
    location: 'New York, USA',
    performanceScore: 91,
    rating: 4.8,
    completedProjects: 25,
    ongoingProjects: 3,
    salary: 120000,
    lastEvaluationDate: '2024-12-20',
    totalTimeSpentHours: 2500,
    projects: [
      {
        id: 1,
        project_title: 'Website Redesign',
        project_subtitle: 'Modernize corporate website',
        project_project_length: 6,
        project_estimated_date: '2025-06-01',
        project_costing_needed: 50000,
        project_resource_needed: 5,
        project_requirement: [
          'UI/UX Design',
          'Frontend Development',
          'Backend Integration',
        ],
        working_resource: [
          {
            id: 1,
            name: 'Tapash Paul',
            email: 'tapash.paul@techcorp.com',
            time_spent_hours: 140,
            performance_score: 95,
          },
          {
            id: 2,
            name: 'Alice',
            email: 'alice@gmail.com',
            time_spent_hours: 120,
            performance_score: 88,
          },
        ],
        projectStatus: 'start',
      },
      {
        id: 2,
        project_title: 'CRM Integration',
        project_subtitle: 'Connect internal tools with CRM',
        project_project_length: 4,
        project_estimated_date: '2025-04-15',
        project_costing_needed: 30000,
        project_resource_needed: 3,
        project_requirement: ['API Integration', 'Database Sync'],
        working_resource: [
          {
            id: 1,
            name: 'Tapash Paul',
            email: 'tapash.paul@techcorp.com',
            time_spent_hours: 100,
            performance_score: 90,
          },
        ],
        projectStatus: 'completed',
      },
    ],
  },
];

export const menuItems: UserMenu[] = [
  { id: 'basic', label: 'Basic Info' },
  { id: 'projects', label: 'Projects' },
  { id: 'performance', label: 'Performance' },
];
// define user data to show
export const displayFields: DisplayField = {
  name: 'Name',
  email: 'Email',
  role: 'Role',
  department: 'Department',
  phone: 'Phone',
  location: 'Location',
};

export const displayFieldsWithIcon: DisplayFieldWithIcon = {
  name: { label: 'Name', icon: './images/profile.jpg' },
  role: { label: 'Role', icon: faUserGear },
  email: { label: 'Email', icon: faMailBulk },
  phone: { label: 'Phone', icon: faMobilePhone },
  location: { label: 'Location', icon: faMapLocation },
};
